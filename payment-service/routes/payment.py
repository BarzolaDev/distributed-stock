...
def charge_account(account_id: UUID, amount: int, idempotency_key: str, db: Session = Depends(get_db)):
    try:
...
def refund_account(account_id: UUID, amount: int, db: Session = Depends(get_db)):
    try:
